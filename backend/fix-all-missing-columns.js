...
async function fixAllMissingColumns() {
  try {
    ...
    // Fix 5: Free/Premium fields - is_free on courses, plan_type on users
    console.log('\n📋 Adding Free/Premium fields...');
    await fixIsFreeCourses();
    await fixPlanTypeUsers();
...
async function fixIsFreeCourses() {
  try {
    const [columns] = await sequelize.query(`
      SELECT column_name 
      FROM information_schema.columns 
    ...
    if (columns.length > 0) {
      console.log('✅ courses.is_free column already exists');
      return;
    }

    console.log('🔧 Adding is_free column to courses...');
    await sequelize.query(`ALTER TABLE courses ADD COLUMN is_free BOOLEAN NOT NULL DEFAULT false`);

    // Seed: mark Python (3), Machine Learning (4), Demo (7) as free
    await sequelize.query(`UPDATE courses SET is_free = true WHERE id IN (3, 4, 7)`);
    console.log('✅ Added is_free to courses and seeded free courses (IDs: 3, 4, 7)');
  } catch (error) {
    console.error('❌ Error adding is_free to courses:', error.message);
  }
}

async function fixPlanTypeUsers() {
  try {
    const [columns] = await sequelize.query(`
      SELECT column_name 
      FROM information_schema.columns 
    ...
    if (columns.length > 0) {
      console.log('✅ users.plan_type column already exists');
      return;
    }

    console.log('🔧 Adding plan_type column to users...');
    // Create ENUM type if not exists
    await sequelize.query(`
      DO $$ BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'enum_users_plan_type') THEN
          CREATE TYPE enum_users_plan_type AS ENUM('free', 'premium');
        END IF;
      END $$;
    `);
    await sequelize.query(`
      ALTER TABLE users ADD COLUMN plan_type enum_users_plan_type NOT NULL DEFAULT 'free'
    `);
    console.log('✅ Added plan_type to users (all existing users set to free)');
  } catch (error) {
    console.error('❌ Error adding plan_type to users:', error.message);
  }
}
