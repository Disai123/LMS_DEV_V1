...
const { jsonArrayGetter, jsonArraySetter } = require('../utils/dialect');
...
module.exports = (sequelize, DataTypes) => {
  const Course = sequelize.define('Course', {
    ...
    tags: {
      type: DataTypes.JSON,
      defaultValue: [],
      get: jsonArrayGetter('tags'),
      set: jsonArraySetter('tags')
    },
    learning_objectives: {
      type: DataTypes.JSON,
      defaultValue: [],
      get: jsonArrayGetter('learning_objectives'),
      set: jsonArraySetter('learning_objectives')
    },
    is_free: {
      type: DataTypes.BOOLEAN,
      defaultValue: false,
  ...
  Course.search = function (searchTerm) {
    ...
    return this.findAll({
      where: {
        is_published: true,
        [Op.or]: [
          ilikeOp(sequelize, 'title', searchTerm),
          ilikeOp(sequelize, 'description', searchTerm),
          tagsSearchLiteral(sequelize, searchTerm)
        ]
      },
      order: [['created_at', 'DESC']]
    });
