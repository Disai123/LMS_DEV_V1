...
const startTest = async (req, res, next) => {
  try {
    ...
    // Check if student is enrolled in the course (admins may test without enrolling)
    ...
    if (enrollment && contentStatuses.includes(enrollment.status) && enrollment.progress < 100) {
    ...
    if (isPassed) {
      ...
      if (existingCertificate) {
        ...
        try {
          ...
          logger.error('Failed to send certificate notification:', notifError);
        }
