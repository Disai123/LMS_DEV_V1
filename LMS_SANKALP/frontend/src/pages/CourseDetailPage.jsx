...
const CourseDetailPage = () => {
  ...
  const previewCourse = courseData?.data?.course
  const enrollmentFromCourseData = courseData?.data?.enrollment

  // Override required_plan from manifest if found
  const meta = getCourseMetadata(previewCourse?.title);
  const effectiveRequiredPlan = meta ? meta.plan : (previewCourse?.required_plan || 'free');
  
  // Custom manifest-based access check
  const isLockedByManifest = meta && !isAdmin && !manifestCheck(userTierOrder, meta.plan);

  const accessLevel = enrollmentFromCourseData
    ? (courseData?.data?.accessLevel === 'locked' ? 'locked' : 'enrolled')
      ...
      <main className="py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
              ...
              <div className="relative p-6">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  <div className="space-y-4">
                    ...
                    >
                      <span className={`inline-flex items-center px-4 py-2 text-xs font-semibold rounded-lg shadow-sm ${course.difficulty === 'beginner'
                    ...
                    {!isAdmin && !isEnrolled && (
                      <motion.div
                        ...
                        animate={{ opacity: 1, scale: 1 }}
                        transition={{ delay: 0.8 }}
                        className={`mt-4 p-4 border rounded-lg shadow-sm ${isPreviewMode
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-3">
                            <div className={`p-2 rounded-lg shadow-md ${isPreviewMode
                            ...
                            <div>
                              <h3 className={`text-sm font-bold ${isPreviewMode ? 'text-blue-800' : 'text-amber-800'
                                }`}>
                                {isPreviewMode ? 'Preview Mode' : 'Enrollment Required'}
                              </h3>
                              <p className={`text-xs ${isPreviewMode ? 'text-blue-600' : 'text-amber-600'
                                }`}>
                                {isPreviewMode
                    ...
                    <div className="flex-1 flex flex-col">
                      {isLocked ? (
                        ...
                        <div className="h-full flex flex-col items-center justify-center p-8 text-center bg-gray-900 text-white relative overflow-hidden">
                          ...
                          <div className="absolute inset-0 bg-gradient-to-b from-transparent via-violet-900/20 to-violet-900/40" />

                          <div className="relative z-10 max-w-md">
                            <div className="w-20 h-20 bg-violet-600/20 border border-violet-500/30 rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-2xl shadow-violet-500/20">
                              <svg className="w-10 h-10 text-violet-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                              </svg>
                            </div>
                            <h2 className="text-3xl font-black mb-4 bg-gradient-to-r from-violet-200 to-indigo-200 bg-clip-text text-transparent italic tracking-tight">
                              PREMIUM CONTENT
                            </h2>
                            <p className="text-slate-400 mb-8 leading-relaxed">
                              This course is exclusive to Premium members. Upgrade your plan to unlock full access to all chapters, videos, and materials.
                            </p>
                            <Link
                              to="/pricing"
                              className="inline-flex items-center px-8 py-4 bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white font-black rounded-xl transition-all duration-300 transform hover:scale-105 shadow-xl shadow-violet-500/25 italic"
                            >
                              UPGRADE TO PREMIUM
            ...
            {/* Test Section - enrolled students and admins */}
            {(isEnrolled && enrollment) || isAdmin ? (
              <TestSection
                courseId={course.id}
