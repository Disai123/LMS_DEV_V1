...
End-to-end guide: EC2 On-Demand + RDS PostgreSQL + Nginx + PM2 + SSL + domain.
...
**Recommendation:** Use **Option A** — single **t3.medium On-Demand** EC2 with Nginx + PM2. Use **RDS** for PostgreSQL (do not run Postgres on the same EC2 in production if you can avoid it). Keep using **S3** for uploads (already configured in backend).

**On-Demand billing:** You pay **per hour** while the instance is **running**. No upfront commitment. Launch from **EC2 → Launch instance** and leave the default **On-Demand** pricing — no need to buy Reserved Instances or Savings Plans. To reduce cost when not in use, **Stop** the instance (you still pay for EBS storage; Elastic IP may incur a small charge if not attached).
...
| Resource                           | Purpose                                         |
| ---------------------------------- | ----------------------------------------------- |
| EC2 `t3.medium` (On-Demand)        | App server (Nginx + Node + built frontend)      |
...
### Phase 3 — EC2 instance (On-Demand)
...
5. **Pricing / purchase option:** **On-Demand** (default — do not select Spot or Reserved here)
6. **Key pair:** create/download `.pem` (e.g. `gnanamai-lms.pem`)
7. **Network:** same VPC as RDS; **Auto-assign public IP:** Enable (or use only Elastic IP)
8. **Security group** (create `lms-sg`):
...
9. **Storage:** 30–50 GB gp3
10. **Launch instance** — billing starts when the instance enters **running** state.

**On-Demand tips:**
- **Stop** (not Terminate) when you are not using the server for long periods — you pay only for EBS disk while stopped.
- **Start** again from EC2 console when you need it; Elastic IP stays the same if still associated.
- Monitor spend: **Billing → Cost Explorer** → filter by EC2.
...
## 9. Cost rough estimate (ap-south-1, monthly — On-Demand)


| Item | Approximate | Notes |
| ---- | ----------- | ----- |
| EC2 t3.medium On-Demand (24/7) | ~$30–38/mo | ~$0.0416/hr × 730 hrs; varies by region |
| EC2 t3.medium (stopped) | ~$3–5/mo | EBS storage only (~30–50 GB gp3) |
| RDS db.t3.micro (24/7) | ~$15–20/mo | Free Tier eligible first 12 months on new accounts |
| RDS db.t3.small (24/7) | ~$25–30/mo | Better for production LMS |
| Elastic IP (attached to running instance) | Free | Small charge if allocated but not attached |
| S3 + transfer | Usage-based | Often low for LMS file uploads |
| Route 53 hosted zone | ~$0.50/mo | Per hosted zone |

**Total (typical On-Demand stack):** ~$45–70/mo with EC2 `t3.medium` + RDS `db.t3.small` running 24/7.

**Optional later:** If you run 24/7 for 12+ months, you can buy a **Reserved Instance** or **Savings Plan** to lower EC2 cost — not required for this guide.
...
| EC2 instance type?         | **t3.medium On-Demand** — launch from EC2 console; pay per hour while running. No reservation needed. |
