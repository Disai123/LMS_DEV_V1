...
class PhaseNavigation {
  ...
  getSubphaseDescription(id) {
    const descriptions = {
      ...
      // Prerequisites
      'overview': 'Introduction video and learning objectives'
    };
  ...
  switchTab(tabId) {
    ...
    if (nextBtn) {
      ...
      if (tabId === 'conclusion' || tabId === 'final-steps') {
        ...
        const span = nextBtn.querySelector('span:first-child');
        if (span && span.textContent === 'Proceed to BRD Phase') {
  ...
  async loadContent(tabId) {
    ...
    try {
      ...
      // When there is no API base (e.g. opening the file directly in the
      // browser), first try to read inline content from a <template> embedded
      // in the current page, then fall back to a same-origin fetch.
      if (!apiBase) {
        console.log('[Navigation] Standalone mode – checking for inline content template');

        // 1) Look for an inline <template id="inline-content-{tabId}"> in the DOM
        const inlineTemplate = document.getElementById('inline-content-' + tabId);
        if (inlineTemplate && inlineTemplate.content) {
          const wrapper = document.createElement('div');
          wrapper.appendChild(inlineTemplate.content.cloneNode(true));
          this.contentCache[tabId] = wrapper.innerHTML;
          contentArea.innerHTML = this.contentCache[tabId];
          contentArea.classList.add('content-fade-in');
          console.log('[Navigation] Loaded content from inline template:', tabId);
          return;
        }

        // 2) Fallback: try a relative fetch (works on http:// localhost, not file://)
        try {
          const pageDir = currentUrl.href.substring(0, currentUrl.href.lastIndexOf('/') + 1);
          contentUrl = pageDir + subphase.file;
          console.log('[Navigation] Standalone fetch URL:', contentUrl);
          ...
          contentArea.innerHTML = '<div class="content-section"><p style="color:#991b1b;">Content could not be loaded in standalone mode. Please open via the LMS server.</p></div>';
        }
        return; // done – skip the API path below
      }
      ...
      if (apiBase) {
        ...
        const phaseFolders = {
          ...
          // Prerequisites
          'antigravity': 'Antigravity_Basics',
          'reactjs': 'React_JS_Basics',
          'fastapi': 'FastAPI_Basics',
          'postgresql': 'PostgreSQL_Basics'
        };
  ...
  updatePhaseNavigationBar() {
    ...
    const phases = ['agent-skills', 'brd', 'uiux', 'architectural', 'code-development', 'testing', 'deployment'];
